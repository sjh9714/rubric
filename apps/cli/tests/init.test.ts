...
describe("rubric init", () => {
  it("creates a starter config", async () => {
    ...
    expect(result.stdout).toContain(
      "Edit .rubric/rules to capture one repeated team review comment."
    );
    expect(result.stdout).toContain(
      "Run rubric compile to publish it to agent and PR instructions."
    );
    expect(result.stdout).toContain(
      "Run rubric check --base main before your next PR."
    );
    await expect(
  ...
  it("creates a GitHub workflow", async () => {
    ...
    expect(workflow).not.toContain("sjh9714/rubric/packages/action@v0.3.1");
  });
  ...
  it("creates an opt-in GitHub Action comment workflow", async () => {
    ...
    expect(workflow).toContain("uses: sjh9714/rubric/packages/action@v0.3.1");
